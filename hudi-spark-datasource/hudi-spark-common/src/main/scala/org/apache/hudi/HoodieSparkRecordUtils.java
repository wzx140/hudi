  ...
  static Function<InternalRow, HoodieRecord<InternalRow>> createMapper(Schema schema, Map<String, Object> prop) {
    if (!MapperUtils.hasInfo(prop)) {
      ...
      if (populateMetaFields) {
        return (data) ->
            HoodieSparkRecordUtils.convertToHoodieSparkRecord(schema, data, preCombineField, withOperationField);
      } else if (keyGen.isPresent()) {
        return (data) ->
            HoodieSparkRecordUtils.convertToHoodieSparkRecord(schema, data, preCombineField, keyGen.get(), withOperationField, Option.empty());
      } else {
        return (data) ->
            HoodieSparkRecordUtils.convertToHoodieSparkRecord(schema, data, preCombineField, withOperationField, partitionName);
      }
    }
  }
}
