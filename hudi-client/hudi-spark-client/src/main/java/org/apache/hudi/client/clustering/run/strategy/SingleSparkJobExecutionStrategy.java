    ...
    List<Iterator<HoodieRecord<T>>> iteratorsForPartition = clusteringOps.stream().map(clusteringOp -> {
      ...
      Iterable<HoodieRecord<T>> indexedRecords = () -> {
        try {
          ...
          MappingIterator mappingIterator = new MappingIterator((ClosableIterator<HoodieRecord>) baseFileReader.getRecordIterator(readerSchema, MapperUtils.buildMapperConfig(recordType)),
